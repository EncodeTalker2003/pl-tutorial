...
```k
  ...
  syntax Exp ::= Int | Bool | String | Name | KVar
               | "(" Exp ")"                       [bracket]
...
```k
  syntax priority @__FUN-LETREC-COMMON
                > apply
                > arith
                > _:=__FUN-LETREC-COMMON
                > let_in__FUN-LETREC-COMMON
                  letrec_in__FUN-LETREC-COMMON
                  if_then_else__FUN-LETREC-COMMON
                > _;__FUN-LETREC-COMMON
                > fun__FUN-LETREC-COMMON
                > datatype_=___FUN-LETREC-COMMON
endmodule

module FUN-LETREC-MACROS
  imports FUN-LETREC-COMMON
```
...
```k
  ...
  syntax Name ::= "$x" [token]
  rule letrec Bs in E => let $x = (mu $x -> maketuple(Bs, .Exps)) in let make_projection(Bs) in E'  [macro]
```
