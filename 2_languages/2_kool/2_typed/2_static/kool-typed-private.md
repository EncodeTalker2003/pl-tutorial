...
```k
  ...
  syntax priority _.__KOOL-TYPED-PRIVATE-SYNTAX > _[_]_KOOL-TYPED-PRIVATE-SYNTAX > _(_)_KOOL-TYPED-PRIVATE-SYNTAX
  ...
  configuration <T multiplicity="?" color="yellow">
                  <tasks color="orange" multiplicity="?">
                    <task multiplicity="*" color="yellow" type="Set">
					  ...
					  <privateEnvT multiplicity="?" color="red"> .Set </privateEnvT>
                    </task>
                  </tasks>
...
```k
  ...
  rule <k> private T:Type F:Id(Ps:Params) S
        => checkprivateMethod(F, getTypes(Ps)->T, C')
           ~> getTypes(Ps)->T F; ...</k>
       <inClass> C </inClass>
       <ctenvT> _ </ctenvT> // to ensure we are in a class pass
