  ...
  configuration <T color="red">
                  <threads color="orange">
                    <thread multiplicity="*" type="Set" color="yellow">
					  ...
					  <privateEnv color="LightGray"> .Set </privateEnv>
...
```k
  ...
  rule <k> super . _X ...</k>
       ...
       <envStack> ListItem(envStackFrame(Class':Id,_,_)) => .List ...</envStack>
    requires Class =/=K Class'
```
## Method invocation
...
```k
  ...
  rule <k> (objectClosure(_, EStack) . X
    => lookupMember(EStack, X:Id, CurClass))(_:Exps) ...</k>
	   <crntClass> CurClass:Id </crntClass>
...
```k
  ...
  rule objectClosure(_, (ListItem(envStackFrame(C,_,_)) => .List) _)
...
```k
  ...
  rule <k> lvalue(objectClosure(Class, ListItem(envStackFrame(Class,Env,PE)) EStack)
              . X
              => lookupMember(ListItem(envStackFrame(Class,Env,PE)) EStack,
                              X, CurClass)) ...</k>
	   <crntClass> CurClass </crntClass>
  rule lvalue(objectClosure(Class, (ListItem(envStackFrame(Class':Id,_,_)) => .List) _)
              . _X)
    requires Class =/=K Class'
```
