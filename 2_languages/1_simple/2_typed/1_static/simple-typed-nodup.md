...
```k
module SIMPLE-TYPED-NODUP-SYNTAX
...
module SIMPLE-TYPED-NODUP
  imports SIMPLE-TYPED-NODUP-SYNTAX
  imports DOMAINS
```
...
```k
  rule <task> <k> T:Type F:Id(Ps:Params) S => getTypes(Ps)->T F; ...</k> </task>
       (.Bag => <task>
               <k> mkDecls(Ps) S </k> <tenv> .Map </tenv> <returnType> T </returnType> <declared> .Set </declared>
             </task>)
```
...
```k
  rule <task> <k> stmt => main(.Exps); </k> (.Bag => <tenv> .Map </tenv> <declared> .Set </declared>) </task>
```
...
```k
  ...
  rule <task> <k> {S} => block ...</k> <tenv> Rho </tenv> <declared> _ </declared> R </task>
       (.Bag => <task> <k> S </k> <tenv> Rho </tenv> <declared> .Set </declared> R </task>)
```
